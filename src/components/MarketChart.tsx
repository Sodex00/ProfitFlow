...
function fallbackCandles(base: number, timeframe: Timeframe): Candle[] {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    previous = close
    return { open, high, low, close, openTime: Date.now()+index, closeTime: Date.now()+index }
  })
}

export function MarketChart({ price, trades, timeframe, candles }: { price: number; trades: Trade[]; timeframe: Timeframe; candles?: Candle[] }) {
  ...
  return <div className="market-chart">
    ...
    <div className="chart-times">{[0,.25,.5,.75,1].map((ratio,index)=>{const item=data[Math.min(data.length-1,Math.floor((data.length-1)*ratio))];return <span key={index}>{new Intl.DateTimeFormat('ru-RU',timeframe==='1d'?{day:'2-digit',month:'short'}:{hour:'2-digit',minute:'2-digit'}).format(new Date(item.openTime))}</span>})}</div>
