...
export function TradeModal({ currentPrice, initialSymbol, currency, fxRate, trade, onClose, onCreate }: Props) {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    onCreate({
      ...
      entryPrice: effectiveEntry, amount: Number(budget) / fxRate / effectiveEntry,
      takeProfit: tpPercent ? effectiveEntry * (1 + (side === 'long' ? 1 : -1) * Number(tpPercent)/100) : undefined,
      stopLoss: slPercent ? effectiveEntry * (1 + (side === 'long' ? -1 : 1) * Number(slPercent)/100) : undefined,
  ...
  return <div className="modal-backdrop" onMouseDown={e => e.target === e.currentTarget && onClose()}>
    <form className="modal" onSubmit={submit}>
      ...
      <div className="form-grid">
        ...
        <label>Цена покупки<input type="number" min="0" step="any" value={entry} placeholder={`По рынку: ${selectedMarketPrice}`} onChange={e => setEntry(e.target.value)}/><small>Оставьте пустым для актуальной цены</small></label>
        <label>Сумма покупки, {currency}<input type="number" min="0" step="any" required value={budget} onChange={e => setBudget(e.target.value)}/><small>Будет куплено ≈ {(Number(budget)/fxRate/effectiveEntry).toFixed(8)} {symbol.replace('USDT','')}</small></label>
        <label>Take Profit, %<div className="percent-input"><input type="number" min="0" step="0.1" value={tpPercent} placeholder="Например, 5" onChange={e => setTpPercent(e.target.value)}/><span>%</span></div><small>{tpPercent ? `Цена: ${(effectiveEntry * (1 + (side==='long'?1:-1)*Number(tpPercent)/100)).toFixed(2)}` : 'Процент от цены входа'}</small></label>
        <label>Stop Loss, %<div className="percent-input"><input type="number" min="0" step="0.1" value={slPercent} placeholder="Например, 2" onChange={e => setSlPercent(e.target.value)}/><span>%</span></div><small>{slPercent ? `Цена: ${(effectiveEntry * (1 + (side==='long'?-1:1)*Number(slPercent)/100)).toFixed(2)}` : 'Процент от цены входа'}</small></label>
      </div>
...
export function CloseTradeModal({ trade, currentPrice, onClose, onSubmit }: { trade: Trade; currentPrice: number; onClose: () => void; onSubmit: (price: number) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={e => e.target === e.currentTarget && onClose()}>
    <form className="modal compact" onSubmit={e => { e.preventDefault(); onSubmit(Number(price) || marketPrice) }}>
      ...
      <label>Цена продажи<input type="number" min="0" step="any" value={price} placeholder={`По рынку: ${marketPrice}`} onChange={e => setPrice(e.target.value)}/><small>Если поле пустое, цена зафиксируется по рынку при продаже</small></label>
      <button type="button" className="market-price" onClick={() => setPrice(String(marketPrice))}><i/> По рынку сейчас <strong>${marketPrice.toLocaleString('en-US')}</strong></button>
