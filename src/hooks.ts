...
const FX_FALLBACK: Record<string, number> = { USD: 1, EUR: 0.86, RUB: 79.5 };
...
export function useCandles(symbol: string, timeframe: Timeframe) {
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      ...
      if (!active) return;
      setCandles((previous) =>
        previous.at(-1)?.openTime === next.openTime
          ? [...previous.slice(0, -1), next]
          : [...previous.slice(-59), next],
      );
    };
  ...
  return {
    candles,
    remainingMs: Math.max(0, (candles.at(-1)?.closeTime ?? now) - now),
  };
}
...
export function useFxRate(currency: string) {
  ...
  useEffect(() => {
    ...
    fetch("https://open.er-api.com/v6/latest/USD")
      .then((r) => r.json())
      .then((data) => {
        if (active && data?.rates?.[currency]) setRate(Number(data.rates[currency]));
      })
      .catch(() => setRate(FX_FALLBACK[currency] ?? 1));
    return () => {
      active = false;
    };
  }, [currency]);
